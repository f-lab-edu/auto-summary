...
class SummaryViewModel @Inject constructor(
    ...
    private fun deleteChatSummary(chatSummary: ChatSummary) {
        viewModelScope.launch {
            container.orbit {
                val currentUiState =
                ...
                reduce {
                    state.copy(
                        chatSummaryState = LoadState.Succeeded(
                            data = currentChatSummaries.toList()
                        )
                    )
                }
            }
    ...
    private fun moveToChatSummaryDetailScreen(chatSummary: ChatSummary) {
        viewModelScope.launch {
            container.orbit {
                postSideEffect(
